...
def cleaning_steps(features: list[str]) -> list[CleaningStep]:
    ...
    return [
        ("duplicates", lambda df: df.dropDuplicates()),
        ("nulls", drop_nulls),
...
def main():
    ...
    df = read_raw(spark, raw_path, columns)
    df = cast_numeric(df, numeric_features)
    ...
    steps = cleaning_steps(numeric_features)
    cleaned, stats = clean(df, steps)  
    for row in stats:
    ...
    sample = draw_sample(cleaned, size, seed=config.sampling.seed)
